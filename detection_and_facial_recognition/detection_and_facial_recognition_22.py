...
class reco:
    ...
    def easy_face_reco(self,frame, known_face_encodings, known_face_names):  # fait la comparaison des visages stockés en BD et ceux récupérés en entrée
        ...
        if process_this_frame:
            ...
            face_locations = face_recognition.face_locations(rgb_small_frame) # dédection des différents visages sur la frame
            face_encodings = face_recognition.face_encodings(rgb_small_frame, face_locations, model='large') # Analyse et conversion (encodage) des visages détectés
            ...
            for face_encoding in face_encodings:
                ...
                if True in matches:
                    ...
                    count += 1          
            # Si la liste des visages encodés est non vide
            if len(face_encodings) != 0:   
                if count == len(face_encodings): # Si l'ensemble des visages encodés ne sont pas reconnu par le système
    ...
    def intro(self):
        ...
        while( int(time.time() - start_time) < capture_duration ):
            ...
            if cv2.waitKey(100) & 0xFF == ord('q'):  # pour avoir une vision en continue toutes les 0.1s
                break  
